#!/usr/bin/env sh
# ============================================================================
# install-task.sh — 从 GitHub 官方 release 下载最新版 task（go-task/task）
# 并安装到 ~/.local/bin（可用环境变量 INSTALL_DIR 覆盖）。
# 支持 Linux / macOS（x86_64 / arm64）。Windows 请使用 install-task.bat。
# 用法: sh install-task.sh
# ============================================================================
set -eu
...
detect_os_arch() {
  case "$(uname -s)" in
    ...
    Darwin) OS="darwin" ;;
    *)
      echo "[ERROR] 不支持的系统: $(uname -s)（本脚本仅支持 Linux/macOS，Windows 请用 install-task.bat）" >&2
      exit 1 ;;
  esac
  case "$(uname -m)" in
...
echo "[OK] task 已安装: $INSTALL_DIR/task"
"$INSTALL_DIR/task" --version || true
ensure_path
